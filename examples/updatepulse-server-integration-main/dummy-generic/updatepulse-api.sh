...
url=$(jq -r '.server' "$(cd "$(dirname "$0")" || exit; pwd -P)/updatepulse.json")
url="${url%/}"
...
version=$(jq -r '.packageData.Version' "$(cd "$(dirname "$0")" || exit; pwd -P)/updatepulse.json")
# define license_key from the updatepulse.json file
license_key=$(jq -r '.licenseKey' "$(cd "$(dirname "$0")" || exit; pwd -P)/updatepulse.json")
# define license_signature from the updatepulse.json file
license_signature=$(jq -r '.licenseSignature' "$(cd "$(dirname "$0")" || exit; pwd -P)/updatepulse.json")
...
function install() {
    ...
    jq --indent 4 '.licenseKey = "'"$1"'"' "$(cd "$(dirname "$0")" || exit; pwd -P)/updatepulse.json" > tmp.json && mv tmp.json "$(cd "$(dirname "$0")" || exit; pwd -P)/updatepulse.json"
...
function uninstall() {
    ...
    jq --indent 4 '.licenseKey = ""' "$(cd "$(dirname "$0")" || exit; pwd -P)/updatepulse.json" > tmp.json && mv tmp.json "$(cd "$(dirname "$0")" || exit; pwd -P)/updatepulse.json"
    # remove the license signature from updatepulse.json
    jq --indent 4 '.licenseSignature = ""' "$(cd "$(dirname "$0")" || exit; pwd -P)/updatepulse.json" > tmp.json && mv tmp.json "$(cd "$(dirname "$0")" || exit; pwd -P)/updatepulse.json"
...
function check_for_updates() {
    local response
    # build the request url
    local endpoint="updatepulse-server-update-api"
    local args=(
        "action=get_metadata"
...
function activate_license() {
    ...
    local signature
    # build the request url
    local endpoint="updatepulse-server-license-api"
    local args=(
        "action=activate"
        "license_key=$(urlencode "$license_key")"
    ...
    jq --indent 4 '.licenseSignature = "'"$signature"'"' "$(cd "$(dirname "$0")" || exit; pwd -P)/updatepulse.json" > tmp.json && mv tmp.json "$(cd "$(dirname "$0")" || exit; pwd -P)/updatepulse.json"
...
function deactivate_license() {
    ...
    # remove the license signature from updatepulse.json
    jq --indent 4 'del(.licenseSignature)' "$(cd "$(dirname "$0")" || exit; pwd -P)/updatepulse.json" > tmp.json && mv tmp.json "$(cd "$(dirname "$0")" || exit; pwd -P)/updatepulse.json"
    ...
    if [ "$(printf '%s\n' "$new_version" "$version" | sort -V | tail -n1)" != "$version" ]; then
        ...
        if [ -d "/tmp/$package_name" ]; then
            ...
            for file in "$(cd "$(dirname "$0")" || exit; pwd -P)"/*; do
                ...
                if [[ ! "$file" == "$(cd "$(dirname "$0")" || exit; pwd -P)"/updatepulse* ]] || [[ "$file" == *.json ]]; then
                    rm -rf "$file"
                fi
            done
            ...
            for file in "/tmp/$package_name"/*; do
                ...
                # check if the file does not start with `updatepulse`, or is .json
                if [[ ! "$file" == /tmp/"$package_name"/updatepulse* ]] || [[ "$file" == *.json ]]; then
                    mv "$file" "$(dirname "$0")"
                fi
            done
        ...
        # add the license key to updatepulse.json
        jq --indent 4 '.licenseKey = "'"$license_key"'"' "$(cd "$(dirname "$0")" || exit; pwd -P)/updatepulse.json" > tmp.json && mv tmp.json "$(cd "$(dirname "$0")" || exit; pwd -P)/updatepulse.json"
        # add the license signature to updatepulse.json
        jq --indent 4 '.licenseSignature = "'"$license_signature"'"' "$(cd "$(dirname "$0")" || exit; pwd -P)/updatepulse.json" > tmp.json && mv tmp.json "$(cd "$(dirname "$0")" || exit; pwd -P)/updatepulse.json"
        # remove the zip
        rm "$output_file"
